...
- [LangSmith](https://smith.Opulentia Langchain.com/) for tracing & observability
...
The first step to running Opulentia Canvas locally is to build the application. This is because Opulentia Canvas uses a monorepo setup, and requires workspace dependencies to be build so other packages/apps can access them.
...
- 🎨 Studio UI: https://smith.Opulentia Langchain.com/studio?baseUrl=http://localhost:54367
```

After your LangGraph server is running, execute the following command inside `apps/web` to start the Opulentia Canvas frontend:
...
Opulentia Canvas is designed to be compatible with any LLM model. The current deployment has the following models configured:
...
Opulentia Canvas supports calling local LLMs running on Ollama. This is not enabled in the hosted version of Opulentia Canvas, but you can use this in your own local/deployed Opulentia Canvas instance.
...
Below are some common issues you may run into if running Opulentia Canvas yourself:
...
Below is a list of features we'd like to add to Opulentia Canvas in the near future:

- **Render React in the editor**: Ideally, if you have Opulentia Canvas generate React (or HTML) code, we should be able to render it live in the editor. **Edit**: This is in the planning stage now!
...
We'd like to continue developing and improving Opulentia Canvas, and want your help!
